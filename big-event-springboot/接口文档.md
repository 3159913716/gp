...
- **接口名称**：用户手机号注册
- **接口URL**：`/user/register-by-phone`
- **请求方法**：POST
- **接口描述**：通过手机号和验证码注册新用户账号
- **权限要求**：无
...
- 参数说明：
  - `username`: String, 用户名（5-16位非空字符）
  - `password`: String, 密码（5-16位非空字符）
  - `phone`: String, 手机号（11位有效手机号）
  - `code`: String, 短信验证码（6位数字）
...
#### 业务逻辑描述
验证用户名、密码、手机号和验证码格式，检查用户名和手机号是否已被占用，验证短信验证码有效性，成功后创建新用户并返回成功。
...
### 1.2.1 用户登录接口（手机号验证码方式）

- **接口名称**：用户手机号验证码登录
- **接口URL**：`/user/login-by-phone`
- **请求方法**：POST
- **接口描述**：用户通过手机号和验证码登录认证并获取JWT令牌
- **权限要求**：无
...
#### 业务逻辑描述
验证手机号格式，检查用户是否存在且状态正常，验证短信验证码有效性，成功后更新最后登录时间，生成JWT令牌并存储到Redis中，返回令牌。
...
- **接口名称**：发送修改密码验证码
- **接口URL**：`/user/send-update-pwd-code`
- **请求方法**：POST
- **接口描述**：向当前用户的邮箱或手机号发送用于修改密码的验证码
- **权限要求**：需要登录
...
```json
{
  ...
  "data": {
    ...
    "email": "test@example.com",
    "nickname": "测试用户",
    "userPic": "http://example.com/avatar.jpg"
  }
}
```
...
1. 验证邮箱格式是否正确
2. 根据邮箱查询数据库中是否存在该用户
3. 如果存在，返回用户基本信息（id、username、email、nickname、userPic）
4. 如果不存在，返回"邮箱未注册"错误信息
...
#### 业务逻辑描述
1. 验证请求参数是否为空：type和target都不能为空
2. 验证type是否合法：只能是"email"或"phone"
3. 根据type验证target的格式：
   - 如果type是"email"，验证是否符合邮箱格式（使用正则表达式：^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,64}$）
   - 如果type是"phone"，验证是否符合手机号格式（使用正则表达式：^1[3-9]\\d{9}$）
4. 根据type和target查询数据库中是否存在该用户
5. 如果存在，生成6位数字验证码并发送：
   - 如果type是"email"，调用emailVerifyService发送验证码
   - 如果type是"phone"，调用smsService发送验证码
6. 返回验证码发送结果：
   - 发送成功返回"验证码发送成功"
   - 发送失败返回相应的错误信息
...
#### 业务逻辑描述
1. 验证请求参数是否为空：type、target、code、newPwd、rePwd都不能为空
2. 验证type是否合法：只能是"email"或"phone"
3. 根据type验证target的格式：
   - 如果type是"email"，验证是否符合邮箱格式（使用正则表达式：^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,64}$）
   - 如果type是"phone"，验证是否符合手机号格式（使用正则表达式：^1[3-9]\\d{9}$）
4. 根据type和target查询数据库中是否存在该用户
5. 验证newPwd和rePwd是否一致
6. 验证newPwd格式是否正确：必须是5-16位，由字母、数字或下划线组成（使用正则表达式：^[a-zA-Z0-9_]{5,16}$）
7. 根据type验证验证码是否正确：
   - 如果type是"email"，调用emailVerifyService.verify检查验证码是否有效
   - 如果type是"phone"，调用smsService.checkCode检查验证码是否有效
8. 所有验证通过后，更新用户密码并返回成功
...
- **请求参数**：
  - `pageNum`: Integer, 页码（默认值：1）
  - `pageSize`: Integer, 每页条数（默认值：10）
    ...
    "records": [
      {
        ...
        "state": "已发布",
        "createTime": "2023-01-01 12:00:00",
        "updateTime": "2023-01-02 13:00:00"
      }
    ],
    "total": 1,
    "size": 10,
...
- **请求参数**：
  - `targets`: String, 手机号码
  - `type`: String, 验证码类型（register-注册, login-登录, reset-重置密码）
...
{
  "code": 0,
  "msg": "类型参数错误，必须是register、login或reset",
  "data": null
}
```
...
{
  "code": 0,
  "msg": "验证码发送失败，请稍后重试（同一手机号1分钟内只能发送一次）",
  "data": null
}
```

#### 业务逻辑描述
验证手机号格式和类型参数，检查发送频率限制，生成6位验证码，存储到Redis并设置有效期，调用短信服务发送验证码。

### 8.2 验证短信验证码接口

- **接口名称**：验证短信验证码
- **接口URL**：`/api/sms/verify`
- **请求方法**：POST
- **接口描述**：验证短信验证码是否有效
- **权限要求**：无
...
#### 业务逻辑描述
验证参数格式，从Redis中获取存储的验证码，检查验证码是否正确且在有效期内，验证成功后删除验证码。
